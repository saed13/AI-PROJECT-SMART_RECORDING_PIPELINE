...
if os.path.exists(str(project_path + 'results_pipeline.txt')):
    os.remove(str(project_path + 'results_pipeline.txt'))
...
# print("file name lidar : ",(list(lidar_fc_in_camfc.keys())))
    ...
    for (i, j) in zip((coord_list[0]), (coord_list[1])):
        ...
        file_writer(i,truck_position, j)
    ...
    plt.savefig(os.path.join((label_path_file), 'results.png'), dpi = 300)
    fig.show()
    plt.show()
