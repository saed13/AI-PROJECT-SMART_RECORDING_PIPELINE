...
def detect(weights, source, project, save_txt, save_conf, exist_ok):
    ...
    #save_dir = Path(str((project)) / opt.name, exist_ok=opt.exist_ok)
    save_dir = Path(increment_path(Path(project) / opt.name, exist_ok=exist_ok))
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            cnts, _ = cv2.findContours(da_seg_mask.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    if save_txt:  # Write to file
                        ...
                        with open(str(txt_path + '_conf') + '.txt', 'a') as f:
